...
def wwt_file_path(instance, filename):
    created_by = str(instance.created_by)
    return '/'.join(['wwt_files', created_by, instance.filename, filename])

class File(models.Model):
    filename = models.CharField(max_length=255, help_text="file အမည်")
    file = models.FileField(upload_to=wwt_file_path)
    ...
    created_by = models.ForeignKey(User, editable=False, null=True, blank=True, on_delete=models.CASCADE, related_name="wwt")
